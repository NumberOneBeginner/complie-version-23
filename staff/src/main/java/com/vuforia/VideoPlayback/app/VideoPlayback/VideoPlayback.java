...
import com.vuforia.VideoPlayback.ui.SampleAppMenu.SampleAppMenu;
import com.vuforia.VideoPlayback.ui.SampleAppMenu.SampleAppMenuGroup;
import com.vuforia.VideoPlayback.ui.SampleAppMenu.SampleAppMenuInterface;


// The AR activity for the VideoPlayback sample.
public class VideoPlayback extends BaseActivity implements
SampleApplicationControl, SampleAppMenuInterface
{
	public String arDir=null;
	private static final String LOGTAG = "VideoPlayback";
	private static final String VIDEO_RELATIVE_PATH = "VideoPlayback/";
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		try{
			...
			String arPath=DownloadUtil.getARPath(this);
			...
			if(ARDir.exists()){
				...
				for (File file : fileList) {
					if(file.isFile()){
						...
						if("xml".equalsIgnoreCase(fileName.substring(fileName.lastIndexOf(".")+1))){
			...
			for(int i=0;i<targetNumber;i++){
				String url=vidoeList.get(i).getZipurl();
				mMovieName[i]=VIDEO_RELATIVE_PATH+url.substring(url.lastIndexOf("/")+1);//里边存放的是视频的路径
			}

		}catch(Exception e){
			Log.d(LOGTAG, "get AR config fail",e);
			finish();
			return;
		}
		...
		mWasPlaying = new boolean[targetNumber];
		...
		mGestureDetector.setOnDoubleTapListener(new OnDoubleTapListener()
		{
			...
			{
				//Tracy add, double click to cloes the AR
				finish();
				return false;
			}
	...
	private void loadTextures()
	{
		for(int i = 0;i<targetNumber;i++){
			mTextures.add(Texture.loadTextureFromApk(
					"VideoPlayback/transparent.png", getAssets()));
		}
	...
	public boolean doLoadTrackersData()
	{
		...
		dataSetStaff = imageTracker.createDataSet();
		if (dataSetStaff == null)
		{
		...
		if (!dataSetStaff.load(getArDir()+databaseFileName,
				STORAGE_TYPE.STORAGE_ABSOLUTE))
			//        if (!dataSetStaff.load("redapp.xml",
		...
		if (!imageTracker.activateDataSet(dataSetStaff))
		{
	...
	public boolean doUnloadTrackersData()
	{
		...
		if (dataSetStaff != null)
		{
			if (imageTracker.getActiveDataSet() == dataSetStaff
					&& !imageTracker.deactivateDataSet(dataSetStaff))
			{
				Log.d(
						LOGTAG,
						"Failed to destroy the tracking data set Staff because the data set could not be deactivated.");
				result = false;
			} else if (!imageTracker.destroyDataSet(dataSetStaff))
			{
				Log.d(LOGTAG,
						"Failed to destroy the tracking data set Staff.");
				result = false;
			}
	...
	public void onInitARDone(SampleApplicationException exception)
	{
		...
		if (exception == null)
		{
			...
			//Tracy disable the menu as required
	...
	public boolean menuProcess(int command)
	{
		...
		case CMD_EXTENDED_TRACKING:
			for (int tIdx = 0; tIdx < dataSetStaff
					.getNumTrackables(); tIdx++)
			{
				Trackable trackable = dataSetStaff
						.getTrackable(tIdx);
	...
	public String getArDir(){
		return getFilesDir().getAbsolutePath()+"/AR/";
	}
}
