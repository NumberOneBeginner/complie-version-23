				...
				// ICS device
			{
				...
				} else
				{
					try
					{
						...
						Log.d(LOGTAG, "load video "+filename);
						...
						File file=new File(((VideoPlayback)mParentActivity).getArDir()+filename);
						if(!file.exists()){
							throw new Exception("no video file "+file.getName());
						}else{
							Log.d(LOGTAG, "video file exist "+file.getName());
						}
						String filePath=((VideoPlayback)mParentActivity).getArDir()+filename;
