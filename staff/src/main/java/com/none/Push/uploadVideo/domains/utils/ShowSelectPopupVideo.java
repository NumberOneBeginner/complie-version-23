...
public class ShowSelectPopupVideo  extends PopupWindow{
	...
	//定义弹窗子类项列表
	private ArrayList<FolderVideo> mActionItems = new ArrayList<FolderVideo>();
	private Button bt1;
	private boolean IsDiplay;

	public ShowSelectPopupVideo(Context context){
		//设置布局的参数
		this(context, LayoutParams.MATCH_PARENT, 80);
	}


	public ShowSelectPopupVideo(Context context, int width, int height) {
	...
	public void show(View view){
		...
		if(mActionItems.size()==0||mActionItems==null){
			IsDiplay=false;	
		}else{
			IsDiplay=true;	
		}
	...
	private void populateActions(){
		...
		mListView.setAdapter(new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				FolderVideo item = mActionItems.get(position);
				//设置文本文字
				viewHolder.show_select_tv.setText(item.name+"("+item.videos.size()+"Videos )");
		...
		if(mListView.getAdapter().getCount()==2){
			...
			ViewGroup.LayoutParams params = mListView.getLayoutParams();
			params.height=DisplayUtil.dip2px(mContext, 120);

			mListView.setLayoutParams(params);

		}else if(mListView.getAdapter().getCount()==1){

			ViewGroup.LayoutParams params = mListView.getLayoutParams();

			params.height=DisplayUtil.dip2px(mContext, 60);

			mListView.setLayoutParams(params);
		}
		//		}else if(mListView.getAdapter()==null||mListView.getAdapter().getCount()==0){
		//			
		//			ViewGroup.LayoutParams params = mListView.getLayoutParams();
		//
		//			params.height=0;
	 ...
	 * 添加子类项
	 */
	public void addAction(FolderVideo action){
		if(action != null){
	 ...
	 * 根据位置得到子类项
	 */
	public FolderVideo getAction(int position){
