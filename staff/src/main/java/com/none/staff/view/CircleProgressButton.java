...
//				R.styleable.CircleProgressBar_Paint_Width, DEFAULT_PAINT_WIDTH); // 获取画笔宽度
		 // 获取画笔宽度
		int paintWidth  = array.getDimensionPixelSize(R.styleable.CircleProgressBar_Paint_Width, (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_PAINT_WIDTH, getResources().getDisplayMetrics())) ;
...
//				DEFAULT_INSIDE_VALUE);// 圆环缩进距离
		// 圆环缩进距离
		mCircleAttribute.mSidePaintInterval = array.getDimensionPixelSize(R.styleable.CircleProgressBar_Inside_Interval, (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_INSIDE_VALUE, getResources().getDisplayMetrics())) ;
