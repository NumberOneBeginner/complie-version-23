...
public class PcmPlayer {
    ...
    public PcmPlayer(Callback callback, int sampleRate, int channel, int format, int bufferSize) {
        mCallback = callback;
        mAudio = new AudioTrack(AudioManager.STREAM_MUSIC, sampleRate, channel, format, 3 * bufferSize, AudioTrack.MODE_STREAM);
        mState = STATE_STOP;
    }
    ...
    public void start() {
        ...
        if (STATE_STOP == mState && null != mAudio) {
            ...
            if (null != mCallback) {
                    ...
                    if (null != data) {
                        if (null != data.mData) {
                            ...
                            if ( len !=  data.getFilledSize()) {
                                LogHelper.e(TAG, "PcmPlayerTime writedata, write is invalidate len:" + len + "   filledSize:" + data.getFilledSize());
                            }
                ...
                LogHelper.e(TAG, "audio stop1");
                if (null != mAudio) {
                    mAudio.pause();
                    mAudio.flush();
                    mAudio.stop();
                }
                LogHelper.e(TAG, "audio stop2");
                mState = STATE_STOP;
                if (null != mListener) {
