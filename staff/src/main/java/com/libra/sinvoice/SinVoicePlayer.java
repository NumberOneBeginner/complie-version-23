...
public class SinVoicePlayer implements Encoder.Listener, Encoder.Callback, PcmPlayer.Listener, PcmPlayer.Callback {
        ...
        if ( null != text ) {
            ...
            if ( tokenLen > 0 ) {
                ...
                for ( i = 0; i < tokenLen; ++i ) {
                    int index = Common.DEFAULT_CODE_BOOK.indexOf(text.charAt(i));
                    if ( index > -1 ) {
                        tokens[i] = index;
                    } else {
                        break;
    ...
    public void play(final int[] tokens, final int tokenLen, final boolean repeat, final int muteInterval) {
        if (STATE_STOP == mState ) {
            LogHelper.d("gujicheng", "push start-----------");
            ...
            mEncodeThread = new Thread() {
                @Override
                public void run() {
                    mEncoder.start(mSampleRate, mBufferSize, tokens, tokenLen, muteInterval, repeat);
                }
            };
    ...
    public void stop() {
        if (STATE_START == mState) {
            ...
            LogHelper.d("gujicheng", "encode stop");
            ...
            LogHelper.d("gujicheng", "push stop-----------");
        }
    }
    ...
    private void stopEncoder() {
        ...
        if ( null != mEncodeThread ) {
            try {
                LogHelper.d(TAG, "gujicheng join encode thread");
                mEncodeThread.join();
        ...
        LogHelper.d(TAG, "gujicheng end stop encoder");
    }
