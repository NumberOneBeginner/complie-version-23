    ...
    public static void StartServer(String[] args) {
        ...
        for (int i = 0; i < args.length; ++i) {
            if (args[i].equalsIgnoreCase("-h") || args[i].equalsIgnoreCase("--host")) {
                ...
                Log.i("ListBeaconsActivity",args[i + 1]);
        ...
        if(server==null){
            System.out.println("nano server is null");
            server=new SimpleWebServer(host, port, rootDirs, quiet);
        }
        if(!server.wasStarted() || !server.isAlive()){
            System.out.println("nano server is not started");
